...
For the current `0.1.0` release line, the packaged mutating helpers enforce a minimal hard-guard layer:
...
Current package `0.1.0` / protocol `1.0` runtime limits:
...
- Purpose: run a quick continuity freshness check before trusting existing context
- Typical use: cold start or before a major write
- Writes files: no
...
1. Run `preflight_context_check.py --json`.
...
- `preflight_context_check.py`, `archive_logs.py`, and bridge guidance are the main helper surfaces that explicitly call out `update_protocol.md` review in the current `0.1.0` release line
...
- Writes files: yes
- Safety model: preview-first; refuses unknown assets unless `--force` is explicitly passed; refuses sidecar removal while managed bridge blocks still exist in root entry files
- Uninstall note: the current `0.1.0` release line keeps uninstall as a two-step flow when root entry files also contain managed bridge blocks. Remove bridge blocks first, then remove the sidecar
...
- Writes files: yes
- Safety model: preview-first; only supported root entry files are allowed; malformed existing bridge blocks are rejected fail-closed; the current `0.1.0` release line accepts exactly one bridge target per invocation
- Concurrency boundary: do not run concurrently with any other mutating helper on the same project
