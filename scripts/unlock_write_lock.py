#!/usr/bin/env python3
"""Inspect or remove a ContextWeave project write lock."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect or remove a ContextWeave project write lock."
    )
    parser.add_argument(
        "path",
        nargs="?",
...
def main() -> None:
    ...
    if args.json:
        ...
        if not lock_exists:
            print(f"No ContextWeave write lock found at {lock_path}.")
        elif args.yes:
            print(f"Removed ContextWeave write lock: {lock_path}")
        else:
            print(f"ContextWeave write lock found: {lock_path}")
            if lock_payload:
