#!/usr/bin/env python3
"""Validate a ContextWeave workspace and its managed file contracts."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a ContextWeave workspace and its managed file contracts."
    )
    parser.add_argument(
        "path",
        nargs="?",
...
def validate_marker(path: Path, text: str, expected_file_key: str, workspace, findings: list[ValidationFinding]):
    ...
    if marker is None:
        add_finding(findings, "error", "missing_file_marker", "Missing ContextWeave file marker", path)
        return None
    ...
    for rel_path in ROOT_ENTRY_CANDIDATES:
        ...
        if not ok:
            add_finding(
                ...
                "ContextWeave managed bridge block is malformed or incomplete",
                path,
            )
            continue

        has_bridge_block = "<!-- ContextWeave managed bridge start -->" in read_text(path)
...
def validate_exclude_block(workspace, state: dict | None, findings: list[ValidationFinding]) -> None:
    ...
    if not ok:
        add_finding(
            ...
            "ContextWeave managed exclude block is malformed or incomplete",
            exclude_path,
        )
        return
    ...
    if expected_mode == "managed":
        ...
        elif ".contextweave/" not in block_text:
            add_finding(
                findings,
                "error",
                "git_exclude_entry_missing",
                "state.json expects a managed git exclude entry for .contextweave/, but it is missing from the managed block",
                exclude_path,
            )
    elif has_block:
        add_finding(
...
def main() -> None:
    ...
    if workspace is None:
        exit_with_cli_error(
            ...
            json_mode=args.json,
            exit_code=1,
            message="No ContextWeave project root found.",
            payload={"valid": False},
        )
