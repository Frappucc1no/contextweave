...
ContextWeave defines a lightweight, file-native protocol for preserving project continuity across sessions.
...
ContextWeave separates project memory into three distinct roles.
...
ContextWeave uses one and only one storage root per project.
...
- `hidden` -> `PROJECT_ROOT/.contextweave/`
- `visible` -> `PROJECT_ROOT/contextweave/`
...
2. check for `PROJECT_ROOT/.contextweave/config.json`
3. check for `PROJECT_ROOT/contextweave/config.json`
4. if exactly one exists, that storage root is authoritative
5. if neither exists, there is no initialized ContextWeave workspace
6. if both exist, that is an invalid conflict state and tools should stop with a clear error

ContextWeave does not support silently preferring one sidecar over the other when both are present.

The storage root is reserved for ContextWeave-managed assets.
...
For the current `0.2.1` release line on protocol `1.0`, the package-level declaration source for managed storage-root assets is:
...
For the current `0.2.1` release line, one additional optional managed namespace is recognized:
...
Current recognized subtree in this `0.2.1` release line:
...
ContextWeave assumes multiple sessions may touch the same project over time.
...
For packaged helper writes in the current `0.2.1` release line, ContextWeave also uses:
...
ContextWeave is intentionally designed so that not everything needs to be read at once.
...
ContextWeave uses a machine-readable contract layer for managed workspace files.
...
```text
<!-- contextweave:file=<file_key> version=<protocol_version> lang=<workspace_language> -->
```
...
```text
<!-- last-writer: [ContextWeave] | 2026-04-07 -->
```
...
For protocol `1.0`, ContextWeave is a two-language system.

It does not claim generic locale support beyond the values listed above.

Version compatibility rules:

- `config.json.protocol_version` must use a supported ContextWeave protocol version
- every managed file marker `version` must use a supported ContextWeave protocol version
...
- `.contextweave/config.json` implies `storage_mode=hidden`
- `contextweave/config.json` implies `storage_mode=visible`
...
This allows ContextWeave to support:
...
ContextWeave also uses managed block boundaries for two integration surfaces:
...
Current supported root entry files for the current `0.2.1` release line:
...
These markers define the ContextWeave-managed region.
