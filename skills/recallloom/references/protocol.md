...
For the current `0.2.2` release line on protocol `1.0`, the package-level declaration source for managed storage-root assets is:
...
For the current `0.2.2` release line, one additional optional managed namespace is recognized:
...
For packaged helper writes in the current `0.2.2` release line, RecallLoom also uses:
...
Current supported root entry files for the current `0.2.2` release line:
